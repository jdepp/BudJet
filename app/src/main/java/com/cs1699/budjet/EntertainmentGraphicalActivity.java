                        ...
                        for (Map.Entry<String, Income> income : incomes.entrySet()) {  // Iterate through the hashmap of incomes and append each income to a string to be displayed
                            if(income.getValue().getCategory().getName().toString().equalsIgnoreCase("entertainment")){
                                if(income.getValue().isRecurring()) {
                                    ...
                                    if(income.getValue().getRecurRate() == 1) { inc_entertainment_sum += (income.getValue().getValue() * (i_days - i_day)); }
                                    else if(income.getValue().getRecurRate() == 2) { inc_entertainment_sum += (income.getValue().getValue() * Math.floor((i_days-i_day)/7)); }
                                    else if(income.getValue().getRecurRate() == 3) { inc_entertainment_sum += income.getValue().getValue(); }
                                }
                                else { inc_entertainment_sum += income.getValue().getValue(); }
                            }
                        }
                        ...
                        for (Map.Entry<String, Expense> expense : expenses.entrySet()) {  // This is how we're going to iterate thru the user's hashpmap properties
                            if(expense.getValue().getCategory().getName().toString().equalsIgnoreCase("entertainment")){
                                if(expense.getValue().isRecurring()) {
                                    ...
                                    if(expense.getValue().getRecurRate() == 1) { exp_entertainment_sum += expense.getValue().getValue() * (e_days-e_day); }                       // Daily
                                    else if(expense.getValue().getRecurRate() == 2) { exp_entertainment_sum += expense.getValue().getValue() * Math.floor((e_days-e_day)/7); }    // Weekly
                                    else if(expense.getValue().getRecurRate() == 3) { exp_entertainment_sum += expense.getValue().getValue(); }                                   // Monthly
                                }
                                else { exp_entertainment_sum += expense.getValue().getValue(); }
                            }
                        }
